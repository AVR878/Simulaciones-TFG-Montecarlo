#Primero instalamos, si hiciera falta, y cargamos la librerías que vamos a emplear:

#INSTALACIÓN:

paquetes <- c("ggplot2", "tseries", "randtests", "randtoolbox")
...
generador_congruencial=function(a,c,M,semilla,N){
  ...
  semilla=semilla %% M #Sacamos el residuo por si acaso nos pasamos del módulo
  
  muestra=c(semilla)#Empezamos la muestra con el valor escogido de la semilla.
  
  if(N>1){#Controlamos el caso de que se pase por tamaño de muestra el 1
...
h <- hist(muestra_pseudo_unif, breaks = 20, plot = FALSE)
...
ggplot(df, aes(x = muestra)) +
  ...
  labs(
    title = "Histograma vs densidad teórica U(0,1)",
    x = "Valor",
    y = "Densidad"
  ) +
  theme_gray()
