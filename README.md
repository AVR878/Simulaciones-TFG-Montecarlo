...
- **Simulación II**: basada en la ejecución de tres técnicas de integración unidimensional de Montecarlo, para estimar la integral $C(1)$ de *Fresnel*.
...
- **Primera simulación**: la ejecución debe hacerse de forma secuencial ejecutando, en primer lugar, el script *Muestra_pseudo_uniforme.R* para, acto seguido, ejecutar el script *Muestra_pseudo_normal.R* que nos devuelve la muestra objetivo. La razón principal de esta separación, radica en que para obtener la muestra pseudonormal necesitamos, previamente, simular una muestra pseudouniforme; en consecuencia, como cada muestra requiere su propio análisis de calidad y bondad, se ha decidido separar la ejecución en estos dos scripts.

- **Segunda simulación**: a diferencia de la primera, los scripts de la segunda se pueden ejecutar de forma independiente; concretamente, el script *Calculos_Simpson_simulacion_integral.R* contiene los cálculos para aproximar, con la *regla de Simpson*, la integral objetivo en aras de tener un valor referencial mientras que, por su parte, el script *Simulacion_Integral_Fresnel_coseno.R* posee, integramente, todo el análisis de convergencia y residuos de los tres métodos seleccionado para la simulación.
