...
tamaño_fijo=4000#Fijamos el tamaño muestral común
replicas=250#Indicamos cuántos errores tomaremos de cada método
...
for (i in 1:replicas) {#Nótese que lo que hace diferente cada error es la distinta elección de la semilla de cada generador en cada réplica.
  
  error_as_clasico=classiccal_montecarlo(muestra_uniforme = generador_congruencial(semilla = 17*i,N=tamaño_fijo))-valor_real_integral
  ...
  error_as_IS=importance_sampling_montecarlo(muestra = generador_auxiliar(muestra_uniforme = generador_congruencial(semilla = 89*i,N=3*tamaño_fijo),size=tamaño_fijo))-valor_real_integral
...
p_hist=ggplot(error_asintotico_por_metodo, aes(x = Error_estandarizado)) +
  geom_histogram(
...
p_qq=ggplot(error_asintotico_por_metodo, aes(sample = Error_estandarizado)) +
